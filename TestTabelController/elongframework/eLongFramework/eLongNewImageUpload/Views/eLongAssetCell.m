...
- (void)configCellWithItem:(eLongAssetModel * _Nonnull )item {
    _asset = item;
    switch (item.type) {
        case eLongAssetTypeVideo:
            self.videoView.hidden = YES;
            break;
        case eLongAssetTypeAudio:
            self.videoView.hidden = NO;
            self.videoTimeLabel.text = item.timeLength;
            break;
        case eLongAssetTypeLivePhoto:
        case eLongAssetTypePhoto:
            self.videoView.hidden = YES;
            break;
    }
...
- (void) setCanSelect:(BOOL)canSelect_{
    _canSelect = canSelect_;
    if (_canSelect) {
...
- (IBAction)_handleButtonAction:(UIButton *)sender {
    ...
    if (self.photoStateButton.selected) {
        [UIView animationWithLayer:self.photoStateButton.layer type:eLongAnimationTypeBigger];
    }
