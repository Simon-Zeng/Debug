...
+ (BOOL)routeURL:(NSURL *)URL withController:(eLongRoutes *)routesController parameters:(NSDictionary *)parameters;
- (BOOL)isGlobalRoutesController;

@end


@interface NSString (eLongRoutes)

- (NSString *)eLongRoutes_URLDecodedString;
- (NSDictionary *)eLongRoutes_URLParameterDictionary;

@end


@implementation NSString (eLongRoutes)
...
- (NSDictionary *)eLongRoutes_URLParameterDictionary {
    ...
    if (self.length && [self rangeOfString:@"="].location != NSNotFound) {
        ...
        for (NSString *keyValuePair in keyValuePairs) {
            ...
            parameters[pair[0]] = [paramValue eLongRoutes_URLDecodedString] ?: @"";
        }
    }
    return parameters;
}
...
@interface _eLongRoute : NSObject

@property (nonatomic, weak) eLongRoutes *parentRoutesController;
    ...
    if (componentCountEqual || routeContainsWildcard) {
        ...
        for (NSString *patternComponent in self.patternPathComponents) {
            ...
            if ([patternComponent hasPrefix:@":"]) {
                ...
                NSString *urlDecodedVariableValue = [variableValue eLongRoutes_URLDecodedString];
                ...
                // match wildcards
                variables[eLongRoutesWildcardComponentsKey] = [URLComponents subarrayWithRange:NSMakeRange(componentIndex, URLComponents.count-componentIndex)];
                isMatch = YES;
                break;
...
+ (instancetype)globalRoutes {
    return [self routesForScheme:eLongRoutesGlobalNamespaceKey];
}

+ (instancetype)routesForScheme:(NSString *)scheme {
    eLongRoutes *routesController = nil;
...
- (void)addRoute:(NSString *)routePattern priority:(NSUInteger)priority handler:(BOOL (^)(NSDictionary *parameters))handlerBlock {
    _eLongRoute *route = [[_eLongRoute alloc] init];
    ...
    if (priority == 0 || self.routes.count == 0) {
        ...
        for (_eLongRoute *existingRoute in existingRoutes) {
...
+ (void)removeRoute:(NSString *)routePattern {
    [[eLongRoutes globalRoutes] removeRoute:routePattern];
}
...
- (void)removeRoute:(NSString *)routePattern {
    ...
    for (_eLongRoute *route in self.routes) {
...
+ (void)removeAllRoutes {
    [[eLongRoutes globalRoutes] removeAllRoutes];
}
...
+ (BOOL)routeURL:(NSURL *)URL withParameters:(NSDictionary *)parameters executeRouteBlock:(BOOL)execute {
    ...
    eLongRoutes *routesController = routeControllersMap[[URL scheme]] ?: [self globalRoutes];
...
+ (NSString *)description {
    ...
    for (NSString *routesNamespace in routeControllersMap) {
        eLongRoutes *routesController = routeControllersMap[routesNamespace];
...
+ (BOOL)routeURL:(NSURL *)URL withController:(eLongRoutes *)routesController {
...
// 路由
+ (BOOL)routeURL:(NSURL *)URL withController:(eLongRoutes *)routesController parameters:(NSDictionary *)parameters executeBlock:(BOOL)executeBlock {
    [self verboseLogWithFormat:@"Trying to route URL %@", URL];
    BOOL didRoute = NO;
    NSArray *routes = routesController.routes;
    NSDictionary *queryParameters = [URL.query eLongRoutes_URLParameterDictionary];
    [self verboseLogWithFormat:@"Parsed query parameters: %@", queryParameters];
    
    NSDictionary *fragmentParameters = [URL.fragment eLongRoutes_URLParameterDictionary];
    ...
    for (_eLongRoute *route in routes) {
        ...
        if (matchParameters) {
            ...
            finalParameters[eLongRoutesPatternKey] = route.pattern;
            finalParameters[eLongRoutesURLKey] = URL;
            __strong __typeof(route.parentRoutesController) strongParentRoutesController = route.parentRoutesController;
            finalParameters[eLongRoutesNamespaceKey] = strongParentRoutesController.namespaceKey ?: [NSNull null];
...
- (BOOL)isGlobalRoutesController {
    return [self.namespaceKey isEqualToString:eLongRoutesGlobalNamespaceKey];
}
...
#pragma clang diagnostic pop
        ...
        va_end(argsList);
        NSLog(@"[eLongRoutes]: %@", formattedLogMessage);
    }
}
