...
@implementation eLongNetworkSerialization
+(NSString *)jsonStringWithObject:(id)object
{
    ...
    if ([object  isKindOfClass:[NSDictionary  class]]) {
        ...
        if ([eLongNetworkSerialization  adjustIsIos5]) {
    ...
    {  //自定义的model类,先将其转为字典
        NSArray *listAr = [eLongNetworkSerialization getPropertyList:object];
        if (listAr &&listAr.count > 0)
        {
           NSDictionary  *parseDic = [eLongNetworkSerialization  convertObjectFromGievnPropertyList:listAr with:object];
...
+(id)jsonObjectWithData:(NSData *)data
{
    ...
    if ([self  adjustIsIos5]) {
        ...
        if (object == nil) {
            ...
            if (jsonStr.length) {
                jsonStr = [eLongNetworkSerialization removeUnescapedCharacter:jsonStr];
    ...
    if (list && list.count>0) {
        for (NSString *key in list) {
            ...
            if (![subObj isKindOfClass:[NSString class]] &&![subObj  isKindOfClass:[NSDictionary  class]]&&![subObj isKindOfClass:[NSArray  class]]) {
                ...
                if (subObj)
                {
                    dddic = (NSDictionary *)[self  convertObjectFromGievnPropertyList:[eLongNetworkSerialization  getPropertyList:subObj] with:subObj];
                }
            }
