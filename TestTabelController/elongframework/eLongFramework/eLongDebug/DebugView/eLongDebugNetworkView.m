...
@interface eLongDebugNetworkView()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate>
@property (nonatomic,strong) UITableView *networkTableView;
@property (nonatomic,strong) eLongDebugNetwork *debugNetwork;
@property (nonatomic,strong) NSArray *requests;
@property (nonatomic,strong) NSArray *filterRequests;
@property (nonatomic,strong) UITextView *detailTextView;
@end

@implementation eLongDebugNetworkView

- (id) initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        self.debugNetwork = [eLongDebugManager networkInstance];
        // 请求列表
...
- (BOOL) textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (text && text.length) {
        self.filterRequests = [self.requests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
            eLongDebugNetworkModel *model = (eLongDebugNetworkModel *)evaluatedObject;
...
//        cell.detailTextLabel.highlightedTextColor = [UIColor colorWithRed:32.0/255.0 green:32.0/255.0 blue:32.0/255.0 alpha:1];
    ...
    eLongDebugNetworkModel *model = [(self.filterRequests?self.filterRequests:self.requests) objectAtIndex:indexPath.row];
