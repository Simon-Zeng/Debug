...
#import "eLongCustomAB.h"
#import "eLongABContact.h"
//#import "eLongCustomPerson.h"
...
- (void)dealloc {
    searchDisplayCtr.delegate = nil;
    [searchDisplayCtr setSearchResultsDataSource:nil];
    [searchDisplayCtr setSearchResultsDelegate:nil];
    self.delegate = nil;
}
...
- (void)makeTableIndex {
    ...
    for (id person in allPeopleInfos) {
        eLongABContact *contact = [[eLongABContact alloc] initWithRef:(__bridge ABRecordRef)(person)];
    ...
    for (NSString *key in nameIndexDic.allKeys) {
        [nameIndexDic setObject:[[nameIndexDic objectForKey:key] sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            return [((eLongABContact *)obj1).fullName localizedCaseInsensitiveCompare:((eLongABContact *)obj2).fullName];
        }] forKey:key];
    }
}
...
- (id)initWithContactStyle:(ABPropertyID)style {
    ...
    //    if (IOSVersion_6) {
    if (self = [super initWithTitle:@"所有联系人" style:NavBarBtnStyleOnlyBackBtn]) {
        __block eLongCustomAB *controller = self;
            ...
            ABAddressBookRequestAccessWithCompletion(addressBookRef, ^(bool granted, CFErrorRef error)
                                                     {
                                                         if (granted)
                                                         {
                                                             dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                                                                 NSLog(@"granted");
                                                                 allPeopleInfos = (__bridge NSArray *)ABAddressBookCopyArrayOfAllPeople(addressBookRef);
                                                                 ...
                                                                 if (allPeopleInfos) {
                                                                     dispatch_async(dispatch_get_main_queue(), ^{
                                                                         ...
                                                                         [listTable scrollRectToVisible:CGRectMake(0, 0, SCREEN_WIDTH, 371) animated:YES];
    ...
    //        allPeopleInfos = (__bridge NSArray *)ABAddressBookCopyArrayOfAllPeople(addressRef);
    //        [self makeTableIndex];
...
- (void)viewDidLoad {
    ...
    [cancelBtn setTitleColor:COLOR_NAV_BTN_TITLE_H forState:UIControlStateHighlighted];
    ...
    // make search bar
    eLongSearchBarView *searchBar = [[eLongSearchBarView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 44)];
    ...
    listTable = [[UITableView alloc] initWithFrame:CGRectMake(0, searchBar.frame.origin.y + searchBar.frame.size.height, SCREEN_WIDTH, MAINCONTENTHEIGHT - 44)
...
- (void)filterSearchPeopleByName:(NSString *)filterStr {
    ...
    for (eLongABContact *contact in allContactArray) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView == self.searchDisplayController.searchResultsTableView) {
        ...
        UIView* mview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40)];
        mview.backgroundColor = [UIColor colorWithRed:0.89 green:0.89 blue:0.89 alpha:1];
        
        UILabel *headerview = [[UILabel alloc] initWithFrame:CGRectMake(9, -3, SCREEN_WIDTH, 27)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:SelectTableCellKey];
    }
    
    eLongABContact *contact;
    if (tableView == self.searchDisplayController.searchResultsTableView) {
...
//        }else{
//            pvc = [[eLongCustomPerson alloc] initWithTitle:subTitle];
//        }
