...
#import "eLongCommonSelectView.h"
#import "eLongDefine.h"

@interface eLongCommonSelectView ()
@property (nonatomic, retain) UIView *markView;

@end


@implementation eLongCommonSelectView
...
{
    // 生成顶部标题栏
    UIView *topView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, SCREEN_WIDTH, NAVIGATION_BAR_HEIGHT)];
    topView.backgroundColor = RGBACOLOR(248, 248, 248, 1);
    [self addSubview:topView];
    
    UIImageView *shaowView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"basevc_dashed.png"]];
    [shaowView setFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT, 320, 1)];
    [topView addSubview:shaowView];
    
    UIImageView *topShadow = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, SCREEN_SCALE)];
    topShadow.image = [UIImage imageNamed:@"basevc_dashed.png"];
...
{
    return [NSString stringWithFormat:@"%ld%@", (long)(row + _originNumer), _measureWord];
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    pickerRowLabel.text = [NSString stringWithFormat:@"%ld%@", (long)(row + _originNumer), _measureWord];
