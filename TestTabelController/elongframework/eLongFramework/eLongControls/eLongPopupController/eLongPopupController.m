...
- (void)setup {
    ...
    eLongPopupTitleView *titleView = [[self class] commonPopupTitleView];
...
- (void)popupControllerWillPresent:(CNPPopupController *)controller {
    ...
    if (self.style == eLongPopupStylePicker) {
        ...
        if (0 <= _selectedRow && _selectedRow < [self.dataSource numberOfRowInELongPopupController:self]) {
            [self.pickerView selectRow:_selectedRow inComponent:0 animated:NO];
        }
    }
    else {
        [self.tableView reloadData];
        if (0 <= self.selectedRow && self.selectedRow < [self.dataSource numberOfRowInELongPopupController:self]) {
