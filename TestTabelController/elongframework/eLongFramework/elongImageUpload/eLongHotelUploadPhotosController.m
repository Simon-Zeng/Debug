//
//  eLongHotelUploadPhotosController.m
//  ElongClient
//
...
#import "eLongDefineHotelResp.h"
#import "eLongLoadingView.h"
#import "eLongAlertView.h"
//#import "YL_ImageUpManager.h"
//判断公寓的后期加进来lvyue
...
@interface eLongHotelUploadPhotosController()
...
@property (nonatomic, strong) UIView *backgroundPickerView;
@property (nonatomic, strong) UIPickerView *typePicker;
@property (nonatomic, strong) NSMutableArray *pickerViewDatasourceArray;
@property (nonatomic, assign) NSUInteger pickerSelectedIndex;

@property (nonatomic, strong) NSMutableArray *cellImageTypeArray;
@property (nonatomic, strong) NSMutableArray *cellImageNameArray;
@property (nonatomic, strong) NSMutableDictionary *allImageTypeDictionary;
@property (nonatomic, strong) NSMutableArray *uploadStatusArray;
@property (nonatomic, assign) CGPoint lastOffset;

@property (nonatomic, assign) BOOL cancelUpload;
@property (nonatomic, assign) BOOL doneUpload;

@property (nonatomic, strong) NSMutableDictionary *guestRoomDictionary;
...
@implementation eLongHotelUploadPhotosController
...
- (void)dealloc
{
    ...
    for (eLongHttpUploadImage *uploader in _uploaderArray) {
        if (uploader) {
            [uploader stopUpload];
        }
    }
...
- (id) initWithAssets:(NSArray *)assetsArray WithTitleString:(NSString *)titleString hotelDetailModel:(eLongHotelDetailModel *)hotelDetailModel{
    self.hotelDetailModel = hotelDetailModel;
    return [self initWithAssets:assetsArray WithTitleString:titleString];
}

- (id)initWithAssets:(NSArray *)assetsArray WithTitleString:(NSString *)titleString
{

    if(self = [super initWithTitle:titleString style:NavBarBtnStyleOnlyBackBtn]){
        ...
        eLongButtonView *backbtn = [[eLongButtonView alloc] initWithFrame:CGRectMake(0, 0, 48, 34)];
        ...
        NSArray *room = nil;
        //needmodify lvyue
//        if ([YL_ImageUpManager sharedManager].is_upload_from_apartment) {
//            room = [[ApartmentDetailViewController hoteldetail] safeObjectForKey:RespHD_Rooms_A];
//        }else{
//            
//        }
        if (room) {
        ...
        if (self.hotelDetailModel) {            
            for (eLongHotelDetailGroupModel *groupModel in self.hotelDetailModel.productsGroupByRoom) {
                [_guestRoomDictionary setValue:groupModel.RoomInfo.RoomId forKey:groupModel.RoomInfo.Name];
            }
        }
...
//        }
        ...
        // 继续添加
        UIView *goonAddingView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, SCREEN_WIDTH, 88.0f)];
        UIImageView *backgroundView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, SCREEN_WIDTH, 88.0f)];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    eLongHotelUploadImageCell *cell = (eLongHotelUploadImageCell *)[_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_selectedIndex inSection:0]];
    if (cell) {
...
{
    UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, SCREEN_HEIGHT, SCREEN_WIDTH, kPickerViewHeight + NAVIGATION_BAR_HEIGHT)];
    ...
    topSplitView.image = [UIImage noCacheImageNamed:@"basevc_dashed.png"];
    ...
    [leftBtn setTitleColor:COLOR_BTN_TITLE forState:UIControlStateNormal];
    [leftBtn setTitleColor:COLOR_BTN_TITLE_H forState:UIControlStateHighlighted];
    ...
    [rightBtn setTitleColor:COLOR_BTN_TITLE forState:UIControlStateNormal];
    [rightBtn setTitleColor:COLOR_BTN_TITLE_H forState:UIControlStateHighlighted];
...
- (void)showTypePickerView
{
    ...
    [UIView animateWithDuration:0.5f animations:^{
        self.backgroundPickerView.frame = CGRectMake(0.0f, SCREEN_HEIGHT - (kPickerViewHeight + NAVIGATION_BAR_HEIGHT), SCREEN_WIDTH, kPickerViewHeight + NAVIGATION_BAR_HEIGHT);
...
- (void)dismissInView {
    [UIView animateWithDuration:SHOW_WINDOWS_DEFAULT_DURATION delay:0.0f options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.backgroundPickerView.frame = CGRectMake(0.0f, SCREEN_HEIGHT, SCREEN_WIDTH, kPickerViewHeight + NAVIGATION_BAR_HEIGHT);
    } completion:^(BOOL finished) {
    }];
}
...
- (void)cancelUpload:(id)sender
{
    ...
    if (_isDelaying) {
        ...
        [eLongAlertView showAlertTitle:@"" Message:_string(@"tips_dontClickTooOften")];
        return;
    }
    self.isDelaying = YES;
    if (self) {
...
- (void)doAsyncUpload:(NSUInteger)index
{
    @autoreleasepool {
        ...
        if (alAsset == nil || [alAsset isKindOfClass:[NSNull class]]) {
            return;
        }
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            // Compose parameters.
            NSMutableDictionary *hotelDictionary = nil;
            //needmodify lvyue
//            if ([YL_ImageUpManager sharedManager].is_upload_from_apartment) {
//                hotelDictionary = [ApartmentDetailViewController hoteldetail];
//            }else{
//                
//            }
            ...
            if (self.hotelDetailModel) {
                [uploadInfo setValue:self.hotelDetailModel.HotelId forKey:@"hotelId"];
                [uploadInfo setValue:self.hotelDetailModel.HotelName forKey:@"hotelName"];
                [uploadInfo setValue:[NSString stringWithFormat:@"%@%@", self.hotelDetailModel.HotelName, [[eLongAssetsLibraryController shareDataInstance] roomId]] forKey:@"imgName"];
            }else{
            ...
            if (STRINGHASVALUE([[eLongAccountManager userInstance] name])) {
                [uploadInfo setValue:[[eLongAccountManager userInstance] name] forKey:@"uploadAuthor"];
            }
            else {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (_cancelUpload) {
                    ...
                    //                    eLongHttpUploadImage *uploader = (eLongHttpUploadImage *)[_uploaderArray safeObjectAtIndex:index];
                    //                    if (uploader) {
                    //
                    //                    }
                ...
                eLongHttpUploadImage *uploader = (eLongHttpUploadImage *)[_uploaderArray safeObjectAtIndex:index];
                if (uploader == nil) {
                    uploader = [[eLongHttpUploadImage alloc] initWithParamsJsonString:[uploadInfo JSONString] imageAssets:alAsset imageData:imageData];
...
- (void)doUpload:(id)sender
{
    ...
    if ([_assetsPhotoArray count] == 0) {
        ...
        [eLongAlertView showAlertTitle:@"" Message:_string(@"tips_noPictures")];
        return;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //    }
    eLongHotelUploadImageCell *cell = (eLongHotelUploadImageCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if (cell == nil) {
        cell = [[eLongHotelUploadImageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        cell.delegate = self;
        cell.imageName.hidden = YES;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        [cell addSubview:[UIImageView graySeparatorWithFrame:CGRectMake(0.0f, 88.0f - 0.51f, SCREEN_WIDTH, 0.51f)]];
    }
...
#pragma mark - eLongHttpUploadImageDelegate.
...
- (void)imageUploadError:(NSIndexPath *)indexPath error:(NSError *)error
{
    ...
    if (!_isConnectionDisconnect) {
        if ([[error domain] isEqualToString:@"NSURLErrorDomain"]) {
            [eLongAlertView showAlertTitle:@"" Message:[error localizedDescription]];
        }
...
#pragma mark - eLongHotelUploadImageCellDelegate
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        if ([_assetsPhotoArray count] == 30) {
            ...
            [eLongAlertView showAlertTitle:@"" Message:_string(@"tips_pictureIsTooMuch")];
            return;
        }
...
//        NSLog(@">>>>%@", self.titleString);
        ...
        elongAssetsPickerController.hotelDetailModel = self.hotelDetailModel;
        elongAssetsPickerController.invoker = self;
        elongAssetsPickerController.title = @"上传公寓图片";
        // 改为添加的方式
    ...
    [[eLongAssetsLibraryController shareInstance] writeImageToSavedPhotosAlbum:[[info objectForKey:UIImagePickerControllerOriginalImage] CGImage] orientation:(ALAssetOrientation)[[info objectForKey:UIImagePickerControllerOriginalImage] imageOrientation] completionBlock:^(NSURL *assetURL, NSError *error){
        ...
        ALAssetsLibraryAssetForURLResultBlock resultsBlock = ^(ALAsset *asset) {
            if (asset) {
                ...
                [eLongAlertView showAlertTitle:@"" Message:@"存取相册错误, 请检查手机空间是否不足"];
            }
        };
        ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error){
            ...
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"存取相册错误" message:[NSString stringWithFormat:@"请在 设置-隐私-照片中，打开%@的访问权限", APP_NAME] delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alert show];
        };
...
- (void) alertView:(UIAlertView *)alertview clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (10000==alertview.tag) {
        if (self.navigationController.viewControllers.count >= 3) {
            UIViewController * viewController = [self.navigationController.viewControllers safeObjectAtIndex:self.navigationController.viewControllers.count - 3];
            if (viewController) {
...
- (void)ButtonViewIsPressed:(eLongButtonView *)button {
    ...
    if (uploading) {
        //提示语优化by杨泽2014.11.12
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:_string(@"tips_returnsWillCancelTheUpload") message:nil delegate:self cancelButtonTitle:@"继续上传" otherButtonTitles:@"取消返回", nil];
        alert.tag=10000;
        [alert show];
    }
    else {
...
- (void)restoreStateOfButton:(eLongButtonView *)button {
