...
- (id)initWithFrame:(CGRect)frame photoUrls:(NSArray *)urs progress:(BOOL) progress_{
    ...
    if (self) {
        ...
        for (int i = 0; i < PHOTOVIEW_CACHECOUNT; i++) {
            ...
            [imageView eLongLoadImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:i]]
...
- (void)reloadPhotosWith:(NSArray *)urls{
    ...
    for (int i = 0; i < PHOTOVIEW_CACHECOUNT; i++) {
        ...
        [imageView eLongLoadImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:i]] placeholderImage:nil];
    }
...
- (void) photoPageUp{
    ...
    [imageView eLongLoadImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:index]] placeholderImage:nil];
...
- (void)nextPageWithAnimation:(BOOL)animation{
    [photosView setContentOffset:CGPointMake(self.frame.size.width * (page + 1), photosView.contentOffset.y) animated:animation];
}
...
- (void) pageToIndex:(NSInteger)_index{
    ...
    if (_index == 0) {
        ...
        offset = CGPointMake(photoSize.width, 0);
        ...
        offset = CGPointMake(photoSize.width * _index, 0);
        //photosView.contentOffset = CGPointMake(photoSize.width * _index, 0);
    }
    [photosView setContentOffset:offset animated:NO];
}
...
-(void) scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (tpage > page && tpage > 1 && tpage < photoCount - 1) {
        ...
        if ((tpage == 1 || tpage == 0 || tpage == 2) && self.resetFrame) {
            UIImageView *imageView =[photoArray safeObjectAtIndex:0];
            UIScrollView *scrollView = (UIScrollView *)[imageView superview];
            
            [imageView eLongLoadImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:tpage]] placeholderImage:nil];
            scrollView.frame = CGRectMake(tpage * photoSize.width, 0, photoSize.width, photoSize.height);
