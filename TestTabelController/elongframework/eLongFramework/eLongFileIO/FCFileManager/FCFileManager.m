...
+(NSString *) absolutePathWithRelativePath:(NSString *)relativePath{
    ...
    for(NSString *directory in directories)
    {
        ...
        for (NSString * subpath in subPaths) {
            if ([subpath isEqualToString:relativePath]) {
                [absolutePath stringByAppendingFormat:@"%@/%@", directory, relativePath];
                return absolutePath;
            }
        }
...
+(BOOL) isPath:(NSString *)path hasExtension:(NSString *)extension{
    ...
    NSString * subpath = [subpaths lastObject];
...
+ (BOOL)setAttributes:(NSDictionary *)attributes ofItemAtPath:(NSString *)path{
    ...
    BOOL result = NO;
    result = [[NSFileManager defaultManager] setAttributes:attributes ofItemAtPath:path error:nil];
