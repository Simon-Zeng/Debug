...
#define Cache_Hotel_Suggest_Time            7200    // 酒店suggest缓存保留时间（秒）
...
#define Cache_Bank_Time                     7200    // 银行列表缓存时间（秒）
#define Cache_Area_Time                     7200    // 行政区、商圈缓存时间（秒）
#define Cache_Ads_Count                     1       // 广告列表的缓存时间
#define Cache_Ads_Time                      -1      // 不设置缓存时间

#define CanUse_Hotel_SuggestCache           YES
#define CanUse_AreaCache                    YES
#define CanUse_HotelList_Cache              NO
#define CanUse_HotelList_ImageCache         NO
#define CanUse_HotelDetail_Cache            NO
#define CanUse_GrouponList_Cache            NO
#define CanUse_GrouponDetail_Cache          NO
#define CanUse_BankCache                    YES
#define CanUse_AdsCache                     YES
...
#define CACHE_ADS               @"Documents/LocalCache/Ads"
...
//                }else if([key isEqualToString:CACHE_ADS_KEY]){
//                    // 配置首页广告，有值取值，没值取本地默认值
//                    [self configAdsCache:dic];
//                }
//            }
//        }
//        
//        // 首页广告缓存
//        canUseAdsCache                  = CanUse_AdsCache;
//        cacheAdsCount                   = Cache_Ads_Count;
//        cacheAdsTime                    = Cache_Ads_Time;
//    }
//    else {
        // 没有配置，用默认值作为配置
...
- (void)configAdsCache:(NSDictionary *)configDic{
    NSNumber *adsMode = [configDic safeObjectForKey:CACHE_MODE];
    canUseAdsCache = OBJECTISNULL(adsMode) ? CanUse_AdsCache : [adsMode intValue];
    
    NSNumber *adsTime = [configDic safeObjectForKey:CACHE_TIME_MINUTES];
    cacheAdsTime = OBJECTISNULL(adsTime) ? Cache_Ads_Time : [adsTime intValue] * 60;
}
...
- (NSString *)getTimeString {
    ...
    NSString *fixTime = [[formatter stringFromDate:[NSDate date]] stringByAppendingFormat:@"%ld", (long)suffixNum];
...
- (BOOL)removeEarliestDataForDirectoryAtPath:(NSString *)path withFileOutTime:(long long)seconds {
    ...
    return [fileManager removeItemAtPath:[path stringByAppendingPathComponent:_currentDelFile] error:nil];
}
...
- (void)setHotelSuggests:(NSDictionary *)paramDic forCity:(NSString *)cityID {
    if (_canUseCache && canUseHotelSuggestCache) {
...
- (void)setHotelArea:(NSData *)areaData forSearching:(NSString *)searchCondition {
    if (_canUseCache && canUseHotelAreaCache) {
...
- (void)setHotelListData:(NSData *)listData forSearching:(NSString *)searchCondition {
    if (_canUseCache && canUseHotelListCache) {
...
- (void)setHotelListImage:(NSData *)imageData forURL:(NSString *)imageURL {
    if (_canUseCache && canUseHotelListImageCache) {
...
- (void)setHotelDetailData:(NSData *)detailData forSearching:(NSString *)searchCondition {
    if (_canUseCache && canUseHotelDetailCache) {
...
- (void)setGrouponListData:(NSData *)listData forSearching:(NSString *)searchCondition {
    if (_canUseCache && canUseGrouponListCache) {
        if (listData.length > 0) {
...
- (void)setGrouponDetailData:(NSData *)detailData forSearching:(NSString *)searchCondition {
    if (_canUseCache && canUseGrouponDetailCache) {
        if (detailData.length > 0) {
...
- (void)setBankListData:(NSData *)bankData {
    if (_canUseCache && canUseBankCache) {
...
- (void)setAdsData:(NSData *)adsData{
    if (_canUseCache && canUseBankCache) {
        if (adsData.length > 0) {
            if (adsData.length < [device freeDiskSpaceBytes]) {
                NSString *filePath = [NSHomeDirectory() stringByAppendingPathComponent:[CACHE_ADS    stringByAppendingPathExtension:CACHE_EXTENSION]];
                if (![[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:nil]) {
                    NSString * tmp = [filePath stringByDeletingLastPathComponent];
                    [[NSFileManager defaultManager] createDirectoryAtPath:tmp withIntermediateDirectories:YES attributes:nil error:nil];
                }
                [adsData writeToFile:filePath atomically:YES];
            }
            else {
                NSLog(@"no enough space to save bank list file!");
            }
...
- (NSData *) getAdsData{
    if (_canUseCache && canUseAdsCache) {
        // 先打开索引文件查看文件目录
        NSString *filePath = [NSHomeDirectory() stringByAppendingPathComponent:[CACHE_ADS    stringByAppendingPathExtension:CACHE_EXTENSION]];
        if ([fileManager fileExistsAtPath:filePath] && [[filePath pathExtension] isEqualToString:CACHE_EXTENSION]) {
            [self scanFileAtPath:filePath withFileOutTime:cacheAdsTime];
            return [NSData dataWithContentsOfFile:filePath];
        }else{
            NSLog(@"no cache for the ads!");
        }
    }else{
        NSLog(@"cache access is forbidden!!");
    }
